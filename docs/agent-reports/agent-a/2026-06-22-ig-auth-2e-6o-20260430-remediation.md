...
Repository-only remediation for legacy duplicate migration version key `20260430`. The data migration file received a unique **14-digit** version (`20260431120000`) via `git mv` with **byte-for-byte SQL preserved**. Canonical remote history row `20260430` remains mapped to the function migration file unchanged.
...
| `20260430_reclassify_invalid_facebook_dm_threads.sql` → **`20260431120000_reclassify_invalid_facebook_dm_threads.sql`** | `0782ae1a8e4f…` (unchanged) | `UPDATE conversations` reclassify invalid Facebook DM threads | `bc3db79` 2026-04-30 11:10 |
...
→ git mv to 20260431120000_reclassify_invalid_facebook_dm_threads.sql (2E.6O)
```
...
| 2 | `20260431120000_reclassify…` (was shared `20260430`) | `conversations` table + provider thread columns (from earlier foundation) |
| 3 | `20260506…` | Subsequent indexes / provider thread work |

Filename lexicographic sort after remediation:

```text
20260430_add_conversation_ids…  →  20260431120000_reclassify…  →  20260506…
```
...
| **Distinct data migration (orphan version key)** | `20260430_reclassify…` → `20260431120000_reclassify…` | Second commit (11:10); different SQL; remote blank pre-remediation; DATA_STATE_CONFIRMED in production (PR #264) |
...
Version `20260431120000` selected because:
...
- Preserves chronological intent (same-day second migration)
...
| File | Change |
| --- | --- |
| `supabase/migrations/20260431120000_reclassify_invalid_facebook_dm_threads.sql` | Renamed from `20260430_reclassify…` (`git mv`) |
| `src/lib/supabaseMigrationVersionUniqueness.test.ts` | Tests updated for new path + ordering |
...
| Risk | Mitigation |
| --- | --- |
| New version `20260431120000` not yet in remote history | **Expected.** Separate single-version repair authorization required before dry-run PASS — **not** a repeat of 20-version repair |
| Dry-run may list 6 files until `20260431120000` marked applied | Document for operator; data UPDATE is idempotent; production effect already present |
...
```text
migration repair 20260431120000 --status applied --linked
```
