/**
 * Deterministic crop of the official SmartKorp Serie-02 stacked JPEG into a
 * compact transparent sidebar logo PNG (mark above wordmark).
 *
 * Usage: node scripts/brand/crop-smartkorp-sidebar-logo.mjs
 */
import sharp from "sharp";
import { writeFileSync } from "node:fs";

const SRC = "public/brand/AW_SmartKorp_Logo_Serie-02.jpg";
const OUT = "public/brand/smartkorp-sidebar-logo.png";
const META = "public/brand/smartkorp-sidebar-logo.meta.json";
const WHITE = 245;
...
for (let i = 0; i < out.length; i += 4) {
  ...
  const g = out[i + 1];
  const b = out[i + 2];
  if (r >= WHITE && g >= WHITE && b >= WHITE) out[i + 3] = 0;
}
...
const meta = {
  ...
  visibleBoundsInOutput: {
    minX: vminX,
    minY: vminY,
    maxX: vmaxX,
    maxY: vmaxY,
    width: vmaxX - vminX + 1,
    height: vmaxY - vminY + 1
  },
  aspectRatio: +(cropW / cropH).toFixed(4),
  notes: "Stacked mark+wordmark from Serie-02; transparent canvas; sidebar use."
};
