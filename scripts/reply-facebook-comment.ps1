<#
.SYNOPSIS
  ทดลองส่ง reply comment ไป Facebook Page ผ่าน Hub API

.DESCRIPTION
  เรียก POST /api/messages/send โดยใช้ helper:
  - channel = FACEBOOK
  - facebookTargetType = COMMENT
  - facebookTargetId = <COMMENT_ID>

  ใช้ JWT ของผู้ใช้ที่มีสิทธิ์ SALES/MANAGER/ADMIN ใน tenant

.EXAMPLE
  .\scripts\reply-facebook-comment.ps1 `
    ...
    -Content "ขอบคุณสำหรับคอมเมนต์ครับ"
#>
param(
...
if (-not $CommentId) {
  $CommentId = Get-FirstNonEmpty @(
    $env:COMMENT_ID,
    $env:HUB_CHAT_COMMENT_ID
  )
}

$missing = @()
if (-not $BaseUrl) { $missing += "BASE_URL / APP_BASE_URL / NEXT_PUBLIC_APP_URL / VERCEL_URL / -BaseUrl" }
...
if (-not $CommentId) { $missing += "COMMENT_ID / -CommentId" }
...
# channelThreadId: required by older deployed API; matches FacebookAdapter outbound (comment:<id>).
$commentThreadId = "comment:$($CommentId.Trim())"
$body = @{
  tenantId           = $TenantId
  leadId             = $LeadId
  conversationId     = $ConversationId
  channel            = "FACEBOOK"
  channelThreadId    = $commentThreadId
  facebookTargetType = "COMMENT"
  facebookTargetId   = $CommentId
  content            = $Content
} | ConvertTo-Json
