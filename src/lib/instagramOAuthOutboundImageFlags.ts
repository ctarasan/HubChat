...
/** Enables Instagram OAuth outbound image delivery service (default OFF; no worker wiring in IG-AUTH-2E.2). */
export function isInstagramOAuthOutboundImageEnabled(
  env: Record<string, string | undefined> = process.env
): boolean {
  return (
    isInstagramOAuthFoundationEnabled(env) &&
    isInstagramOAuthRuntimeEnabled(env) &&
    readBooleanEnv(env, "HUBCHAT_INSTAGRAM_OAUTH_OUTBOUND_IMAGE_ENABLED")
  );
}
