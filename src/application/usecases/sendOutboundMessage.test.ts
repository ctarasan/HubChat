...
test("outbound provider success is not retried when hubchat.message.sent event logging fails", async () => {
  let sendCount = 0;
  let markSentCount = 0;
  let markProcessedCount = 0;
  const payload: OutboundMessageRequestedPayload = {
    tenantId: "ba82d847-53cd-4b60-9e4d-5fd3f8ad865f",
    leadId: "9e68eadd-01b6-4c66-a522-74b97d6a6902",
  ...
  const useCase = new SendOutboundMessageUseCase({
    channelAdapterRegistry: {
      get: () => ({
        ...
        sendMessage: async () => {
          sendCount += 1;
          return { externalMessageId: "ext-safe-1" };
        },
        fetchUserProfile: async () => ({}),
        fetchConversationThread: async () => []
      })
    ...
    leadEventRepository: {
      create: async () => {
        throw new Error("lead_events table unavailable");
      }
    },
    rateLimiter: { checkOrThrow: async () => {} },
    idempotency: {
  ...
  assert.equal(markProcessedCount, 1);
});
