...
export type FacebookOAuthSessionRouteDeps = {
  apiBootstrap: typeof apiBootstrap;
  requireAuth: typeof requireAuth;
};

export function createFacebookOAuthSessionHandler(
  deps: FacebookOAuthSessionRouteDeps = { apiBootstrap, requireAuth }
) {
  return async function GET(req: NextRequest) {
    try {
      ...
      const data = await service.getSession(auth, resumeHash);
...
export const GET = createFacebookOAuthSessionHandler();
